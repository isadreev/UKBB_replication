...
ress <- filter(res) %>%
  ...
  summarise( n=n(), se=sd(b)/sqrt(n),b=mean(b))


#ress2 <- ress # 0.4
#ress3 <- ress # 
#ress4 <- ress # 0
p1 <- ggplot(ress, aes(x=overlap, y=b, fill=exposure_dataset)) +
...
ggsave(p1, file="replication.png", width=10, height=10)
...
- if there is overlap with the replication then this is as good as having an independent outcome dataset
- any overlap of the outcome with the discovery leads to problems
- higher overlap with the discovery is worse (e.g. if there is partial overlap with discovery and replication, this is better than complete overlap with discovery)


- one sample case (exposure and outcome data from the exposure discovery dataset)
	- this follows standard weak instrument bias - towards the confounder when there is sample overlap, towards the null when there is no sample overlap, in between when there is a mixture of sample overlap and no sample overlap (both_overlap)

- replication exposure data 


- umvcue correction mimics the replication-only result
