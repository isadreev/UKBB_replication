...
gwas_sim <- function(nid = 9000, nsnp = 20, bgx = 0.08, bxy = 0, buy = 0.4, bux = 0.4, out_index = 1:3000)
{
  # Simulate population
  g <- make_geno(nid, nsnp, 0.5)
  u <- rnorm(nid)
...
ggsave(p1, file=paste0(imgdir,"/replication_beta.png"), width=10, height=10)

# Plot proportion of significant p vals
p1 <- ggplot(ress, aes(x=overlap, y=prop_sig, fill=exposure_dataset)) +
geom_bar(stat="identity", position="dodge", aes(fill=exposure_dataset)) +
facet_grid(instruments ~ buy) +
scale_fill_brewer(type="qual") +
theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5))
ggsave(p1, file=paste0(imgdir,"/replication_prop_sig.png"), width=10, height=10)
