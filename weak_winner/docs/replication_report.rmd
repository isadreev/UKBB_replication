...
ress2 <- filter(res) %>%
  ...
  summarise(
    ...
    fdr = sum(pval < 0.05 & bxy == 0) / sum(bxy == 0),
    power = sum(pval < 0.05 & bxy != 0) / sum(bxy != 0)
  )
...
labs(x="False discovery rate (α=0.05)", y="Power (p < 0.05)", shape="Exposure dataset", colour="Outcome dataset")
```
