...
runsim <- function(p)
{
	paramx <- simulateGP::generate_gwas_params(runif(p$nsnp, p$minmaf, p$maxmaf), p$h2, p$S)
	paramy <- paramx
	paramy$beta <- paramy$beta * p$bxy
	ndisc <- round(p$nid * p$propA)
	nrepl <- p$nid - ndisc
	discx <- simulateGP::generate_gwas_ss(paramx$beta, paramx$maf, ndisc)
	replx <- simulateGP::generate_gwas_ss(paramx$beta, paramx$maf, nrepl)
	reply <- simulateGP::generate_gwas_ss(paramy$beta, paramy$maf, nrepl)
...
```{r}
ggplot(lsum, aes(y=z, x=propA, group=as.factor(nsnp * h2))) +
...
```{r}
ggplot(lsum, aes(y=b, x=propA, group=as.factor(nsnp * h2))) +
geom_point(aes(colour=as.factor(nsnp), shape=as.factor(h2))) +
geom_line(aes(colour=as.factor(nsnp), linetype=as.factor(h2)))
```
...
```{r}
ggplot(lsum, aes(y=nsig, x=propA, group=as.factor(nsnp * h2))) +
geom_point(aes(colour=as.factor(nsnp), shape=as.factor(h2))) +
geom_line(aes(colour=as.factor(nsnp), linetype=as.factor(h2)))
```
